...
* Как выглядит `ViT_Encoder_Block`:
    * Применим `LayerNorm` - выход назовем `out1`
...
Реализованный Vision Transformer на датасете Fruits достигает на тесте f1_macro: 0.9545 уже на 5 эпохе.

Реализованный Vision Transformer на датасете Cars достигает на тесте f1_macro: 0.0402 на 40 эпохе.
При этом далее происходит переобучение: уже с 50 эпохи f1_macro на трейне равняется 0.9961, а на тесте f1_macro никак не
растет...
